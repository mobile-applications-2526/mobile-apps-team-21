...
export default function RestaurantOverviewModal({
  ...
  const [selectedSuggestion, setSelectedSuggestion] =
    useState<SuggestedRestaurant | null>(null);
  ...
  const toFeedback = (res: unknown, successDefault: string) => {
    ...
    if (typeof res === "string") {
      ...
      if (lower.startsWith("error")) {
        return {
          message: res.replace(/^error:\s*/i, "").trim(),
          type: "error" as const,
        };
  ...
  const unvote = async (s: SuggestedRestaurant) => {
    try {
      ...
      setFeedback({
        message: e instanceof Error ? e.message : "Unvote failed",
        type: "error",
      });
    }
  };
      ...
      >
        ...
        >
          <Text style={[styles.title, dark && styles.titleDark]}>
            {"Recommended restaurants for " + group.name}
          </Text>

          {restaurants.length === 0 ? (
            <View style={styles.emptyContainer}>
              <Text style={[styles.empty, dark && styles.emptyDark]}>
                No restaurants have been recommended yet.
              </Text>
            </View>
          ) : (
              ...
              {restaurants.map((s) => (
                <View
                  ...
                  style={[
                    ...
                    dark && styles.restaurantCardDark,
                  ]}
                >
                    ...
                    <View style={styles.textContainer}>
                      <Text
                        style={[
                          ...
                          dark && styles.restaurantNameDark,
                        ]}
                      >
                        {s.restaurant.name}
                      </Text>
                      <Text
                        style={[
                          ...
                          dark && styles.restaurantMetaDark,
                        ]}
                      >
                        {s.voters.length} vote(s)
                      </Text>
                      {s.lockedDate ? (
                        <Text
                          style={[
                      ...
                      {s.closed && (
                        <TouchableOpacity
                          onPress={() => {
                            ...
                            const saved: string[] =
                              (s as any).availabilities?.[userEmail || ""] ??
                              [];
                        ...
                        >
                          <MaterialIcons
                            name="calendar-today"
                            size={24}
                            style={[styles.calendarIcon, dark && styles.calendarIconDark]}
                          />
                        </TouchableOpacity>
                      )}
                    </View>
            ...
            >
              <View style={[styles.backdrop, dark && styles.backdropDark]}>
                <View style={[styles.card, dark && styles.cardDark]}>
                  <Text style={[styles.title, dark && styles.titleDark]}>
                    Pick available dates for{" "}
                    {selectedSuggestion.restaurant.name}
                  </Text>
                  <Text
                    style={[
                      styles.restaurantMeta,
                    ...
                    <View style={styles.weekHeader}>
                      {["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"].map(
                        (w) => (
                          <Text
                            key={w}
                            style={[styles.weekday, dark && styles.weekdayDark]}
                          >
                            {w}
                          </Text>
                        )
                    ...
                    <View style={styles.calendarGrid}>
                      {generateNextDays(28).map((d, idx) => {
                        ...
                        return (
                          <TouchableOpacity
                            key={d.iso}
                            onPress={() => toggleSelectDate(d.iso)}
                            style={[
                              styles.calendarCell,
                              dark && styles.calendarCellDark,
                              selected ? styles.calendarCellSelected : null,
                              selected && dark
                                ? styles.calendarCellSelectedDark
                                : null,
                            ]}
                          >
                            <Text
                  ...
                  >
                    ...
                    >
                      <Text
                        style={[styles.forText, dark && styles.forTextDark]}
                      >
                        {selectedDates.length === 0 ? "Save" : "Save"}
                      </Text>
                    </TouchableOpacity>
                  </View>
                </View>
          ...
          <TouchableOpacity
            ...
            disabled={busy}
          >
            <Text style={[styles.cancelText, dark && styles.cancelTextDark]}>
              Close
            </Text>
          </TouchableOpacity>
        </Animated.View>
...
const styles = StyleSheet.create({
  ...
  // calendar cells should not have a background — keep them transparent
  calendarCell: {
    width: "14.28%",
    alignItems: "center",
    paddingVertical: 8,
  ...
  calendarCellSelected: { backgroundColor: "rgba(76, 175, 80, 0.12)" },
  calendarCellSelectedDark: { backgroundColor: "rgba(76, 175, 80, 0.22)" },
  ...
  // calendar-specific button/icon styles
  calendarButton: {
